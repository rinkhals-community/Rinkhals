<script lang="ts">
	...
	// This component is mounted once, permanently, by the root layout - not by
	// the /terminal route. That's deliberate: SvelteKit unmounts routed page
	// components on navigation, which used to destroy every shell (WebSocket +
	// xterm instance) the moment the user clicked away to Logs or Apps. Living
	// in the layout means navigation only toggles this panel's visibility via
	// CSS; the sessions and their sockets keep running underneath.
	...
	// container mounts, and tears everything down when it unmounts (tab closed).
	// The container only unmounts when its tab is closed - navigating to another
	// route does not unmount it, since this whole panel stays mounted.
	function termAttach(node: HTMLElement, id: string) {
		...
		const ro = new ResizeObserver(() => {
			...
			// This also covers the panel itself being hidden on other routes.
	...
	// tabs keep running; their buffers retain output while hidden. Also re-fits
	// when the panel itself becomes visible again (see $effect below, driven by
	// the `active` prop the layout passes in).
	...
	// Open one shell the first time this panel mounts (once per page load, since
	// the panel now lives for the whole session instead of remounting on every
	// visit to /terminal). Closing every tab is allowed and shows an empty
	...
	onDestroy(() => {
		...
		// stragglers if the whole app is torn down (e.g. full page reload).
